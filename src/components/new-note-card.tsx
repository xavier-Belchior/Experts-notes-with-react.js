...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  const [shouldShowOnbarding, setShouldOnbording] = useState(true)
  const [isRecording, setIsRecording] = useState('')
  const [content, setContent] = useState('')

  function handleStartEditor() {
    setShouldOnbording(false)
  }
  function handleContentChanged(event: ChangeEvent<HTMLTextAreaElement>) {
    setContent(event.target.value)
    if (event.target.value == '') {
      setShouldOnbording(true)
    }
  }
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md bg-slate-700 flex flex-col gap-3 p-5 text-left hover:ring-2 hover:ring-slate-600 outline-none focus-visible:ring-2 focus-visible:ring-lime-400  ">
          ...
          <form className="flex-1 flex flex-col">
            ...
            {isRecording ? (
              <button
                ...
                type="button"
                className="w-full flex items-center justify-center gap-2 bg-slate-900 py-4 text-center font-medium text-sm text-slate-300 outline-none hover:text-slate-100"
              >
              ...
              <button
                ...
                type="button"
                className="w-full bg-lime-400 py-4 text-center font-medium text-sm text-lime-950 outline-none hover:bg-lime-500"
              >
                Salvar nota
              </button>
            )}
